...
class Read {
    public static function upsert(int $postId, string $anonId, int $progress, int $timeMs): void {
        $pdo = Database::pdo();
        $stmt = $pdo->prepare("INSERT INTO read_stats (post_id, anon_id, progress, time_spent_ms, created_at, updated_at)
    ...
    public static function get(int $postId, string $anonId): ?array {
        $pdo = Database::pdo();
        $stmt = $pdo->prepare("SELECT * FROM read_stats WHERE post_id = :pid AND anon_id = :aid LIMIT 1");
